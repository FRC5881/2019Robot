...
/**
 * Contains methods to get the error of the motors, set the PIDs and even move the lift and bar!
 */
public class Elevator extends Subsystem {
    private static final double ELEVATOR_STEP = 100;
    private static final double BAR_STEP = 25;
    private static final double BAR_ALLOWED_ERROR = 50;
    private static final double ELEVATOR_ALLOWED_ERROR = 50;

    private static final double BAR_MAX = 1065;
    private static final double ELEVATOR_MAX = 14400;
    ...
    public void init() {
        ...
        // Configures an encoder for the bar motor and lift motor
    ...
    public void manualDrive() {
        double leftTrigger = Robot.oi.driverController.getRawAxis(OI.XBOX_LEFT_TRIGGER);
        double rightTrigger = Robot.oi.driverController.getRawAxis(OI.XBOX_RIGHT_TRIGGER);
        if (leftTrigger > 0.1) {
            barManualDown();
        }
        if (rightTrigger > 0.1) {
    ...
    public void barManualUp() {
        setBarSetpoint(getBarSetpoint() + BAR_STEP);
    }

    public void barManualDown() {
        setBarSetpoint(getBarSetpoint() - BAR_STEP);
    }

    public void elevatorManualUp() {
        setElevatorSetpoint(getElevatorSetpoint() + ELEVATOR_STEP);
    }

    public void elevatorManualDown() {
        setElevatorSetpoint(getElevatorSetpoint() - ELEVATOR_STEP);
    }

    /**
     * Moves the lift up to the next possible level.
     */
    ...
    /**
     * Moves the lift down to the next possible state.
     */
    ...
    /**
     * If elevatorFlip is equal to one thing than set lift to that height
     */
    @SuppressWarnings("Duplicates")
    ...
    /**
     * Sets the lift and bar height to the lowest setting as to prevent a high center of mass
     */
    ...
    /**
     * Checks to see if the lift is within the margin of error of it's target
     *
    ...
    /**
     * Checks to see if the amount of error in the lift motors is acceptable
     *
    ...
    /**
     * Tells the bar and lift to go to the specified height, also sets elevatorState
     *
     * @param state Is the state of which the lift is wanted to go to
     */
     ...
     * @param elevatorSetpoint is the desired setpoint for the lift to go to
     */

    public void setLiftSetpoint(double barSetpoint, double elevatorSetpoint) {
        System.out.println(barSetpoint + " <-- is bar setpoint");
        System.out.println(elevatorSetpoint + " <-- is lift setpoint");
    ...
    /**
     * Gets the value of lift setpoint
     *
     * @return returns the value of lift setpoint
     */
     ...
     *
     * @param setpoint is the height (in ticks) of which you want the lift to go to
     */
    private void setElevatorSetpoint(double setpoint) {
        if ( setpoint > 0 && setpoint < ELEVATOR_MAX) {
    ...
    private void setBarSetpoint(double setpoint) {
       if (setpoint > 0 && setpoint < BAR_MAX) {
    ...
    /**
     * Is the state of the lift and bar
     */
